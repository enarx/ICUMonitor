...
export async function fetchInitialData() {
    const response = await fetch('/api/icu');
    icu = await response.json();
    updateAverageTemperature();
    return icu;
}

export function readings(icu, sensorId) {
    return icu.sensors[sensorId].readings;
}

export function lastReading(icu, sensorId) {
    return readings(icu, sensorId).slice(-1)[0].val;
}
...
export function onDataChanged(callback) {
    ...
    connection.on('addData', data => {
        ...
        data.sensors.forEach(updatedSensor => {
            const existingSensor = icu.sensors[updatedSensor.sensorId];
            if (existingSensor) {
...
function updateAverageTemperature() {
    const series = Object.values(icu.sensors).filter(s => s.sensorId.startsWith('temp-point-')).map(s => s.readings);
