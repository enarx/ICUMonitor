...
{
    public static SensorDataBatch NextDataBatch(Building icu)
    {
        var batch = new SensorDataBatch();
        var latestTimestamp = icu.Sensors.Values.Select(s => s.Readings.Last()).Max(r => r.Timestamp);
        var timestamp = latestTimestamp.AddMinutes(5);

        foreach (var sensor in icu.Sensors.Values)
        {
